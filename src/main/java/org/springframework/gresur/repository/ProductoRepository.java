...
public interface ProductoRepository extends PagingAndSortingRepository<Producto, Long>{
	...
	@Query(value = "SELECT p FROM Producto p ORDER BY ((p.stock * 1.0)+1/p.stockSeguridad) ASC")
	Page<Producto> findAllOrderStock(Pageable pageable);
	
	@Query(value = "SELECT p FROM Producto p INNER JOIN p.estanteria e WHERE e.categoria = :categoria ORDER BY ((p.stock * 1.0)+1/p.stockSeguridad) ASC")
	Page<Producto> findByEstanteriaCategoriaOrderStock(@Param("categoria") Categoria categoria,Pageable pageable); 
	
	@Query(value = "SELECT p FROM Producto p WHERE LOWER(p.nombre) LIKE LOWER(concat('%', concat(:nombre, '%'))) ORDER BY ((p.stock * 1.0 )+1/p.stockSeguridad) ASC")
